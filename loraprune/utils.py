...
def init_sensitivity_dict(model):
    ...
    for name, module in model.named_modules():
        if _is_target_larer(module):
        # if _is_target_layer(module) and'attention' in name or 'intermediate' in name or 'output' in name:
            # 获取模块的输出特征数
...
def update_sensitivity_dict(model, s_dict, prune_metric):
    ...
    for name, module in model.named_modules():
        if _is_target_larer(module):
        # if _is_target_layer(module) and  'attention' in name or 'intermediate' in name or 'output' in name:  # 判断是否是目标模块
            # 判断模块类型
            # print(name)
    ...
    for key, value in importance_scores.items():
        ...
        if "attention" in key:
            if model.config.model_type == 'bert':
                ...
                key = key+'.attention'
                # 对于ViT，key已经包含attention部分，不需要额外添加
                pass  # key保持不变
            mask = mask_dict[key+'.query'].reshape(-1, DIM)[:, 0]#变成头维度
            value =value * mask.to(value.device)
